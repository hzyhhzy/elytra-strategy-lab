...
- [从静止最快到达 `x=1000`，不限制逐帧抖动](docs/minimum-time-x1000.md)：不限制高度并允许逐帧控制，目前验证结果为 `403 tick`（`20.15 秒`）。
- [从静止最快到达 `x=1000`，平滑单调仰角](docs/minimum-time-x1000.md)：要求仰角沿 C2 曲线从 `-90 度` 单调递增到 `0 度`，结果为 `405 tick`（`20.25 秒`）。
- [最大瞬时水平速度](docs/maximum-instantaneous-horizontal-speed.md)：从理想化的竖直终端下落状态出发且不限制高度，确定性高频切换路径达到 `75.196540693 格/秒`，平滑替代达到 `74.444921694 格/秒`。

这些都是非周期研究任务，不属于可直接循环部署的稳态策略。专属文档记录了假设、求解方法、边界检查、图像和 CSV 路径。
...
- `solvers/reachable_peak_speed_convex.cpp`：最大瞬时水平速度的速度可达集凸包搜索。
- `solvers/smooth_cos2_strategy.py`：对峰值速度策略进行 `cos^2(pitch)` 高斯平滑并比较性能。
...
- `scripts/plot_peak_horizontal_speed.py`：重新生成瞬时峰值速度支线的中英文图。
